...
void wifiManagerSetup()
{
  ...
  //wm.resetSettings(); // Uncomment to reset configured wifi

  wm.setConfigPortalBlocking(false);
  wm.autoConnect("ESPD35");  // AP name
  if (!(WiFi.status() == WL_CONNECTED))
  {
    display.fillScreen(TFT_BLACK);
    display.drawString("Connect to ESPD35", 0, 0);
    display.drawString("and configure WIFI", 0, 25);
    display.drawString("IP: 192.168.4.1", 0, 50);
  }
...
void setup() {
  ...
  display.setTextColor(TFT_WHITE, TFT_BLACK);
  display.setTextDatum(TL_DATUM);
  display.drawString("Waiting for WiFi", 0, 0);
